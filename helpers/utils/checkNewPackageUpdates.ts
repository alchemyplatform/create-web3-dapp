...
export const checkNewPackageUpdates = async () => {
    try {
        ...
        console.log(error.stdout.toString().includes("create-next-dapp"));
        if (error.stdout.toString().includes("create-next-dapp")) {
            const wantsToUpdate: boolean | string = await prompts({
                type: "select",
                name: "wantsToUpdate",
                message: "A new version of create-web3-dapp has been found, do you want to update?",
                choices: [
                    {
                        title: "Yes",
                        value: true,
